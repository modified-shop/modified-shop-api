...
/**
 * /includes/external/api/v1/Action/Configuration/ConfigurationGetAction.php
 *
...
trait ConfigurationGetAction
{
    ...
    public function GetConfiguration(int $configurationGroupId): array
    {
        ...
        if (xtc_db_num_rows($configuration_query) < 1) {
            return $this->errormessage(sprintf('Configuration Group not found: %s', $configurationGroupId));
        } else {
            $data = [];
    ...
    public function GetConfigurationGroups(array $options): array
    {
        ...
        if ($count['total'] < 1) {
            return $this->errormessage('no Configuration found');
        }
